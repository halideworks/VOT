...
The diff-scoped CI run also selected the preserved public `path_sample`
wrapper. `public_path_sample_reads_quiches_default_connection` kills both
whole-function replacements by requiring the active path and its nonzero
congestion window and MTU. A targeted rerun selected three `path_sample`
mutants and caught all three.

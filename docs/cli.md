...
Run commands with `cargo run -p vot-cli -- <command>`, or build the executable:

```sh
cargo build -p vot-cli --release --features wire --locked
```

The examples below use `vot-cli` from `target/release` (add that directory to
`PATH`, or use its full path). The `wire` feature needs a C/C++ toolchain,
CMake and libclang; native macOS and Windows builds also need NASM.

## Local: build and verify a bundle

```sh
# Build a deterministic transfer bundle from a directory.
vot-cli send [SUITE] SOURCE_DIR BUNDLE_DIR
# Prints: ROOT LENGTH

# Verify, publish, and write a receipt.
vot-cli receive BUNDLE_DIR DESTINATION_DIR RECEIPT.cbor KEY_SOURCE OBSERVED_AT
# Prints: ROOT LENGTH PUBLISHED

# Check a receipt without the bundle.
vot-cli verify-receipt RECEIPT.cbor KEY_SOURCE
...
vot-cli serve BUNDLE_DIR LISTEN_ADDR [CERT.pem KEY.pem]

# Fetch a bundle from a server. The root is what says which package to accept.
vot-cli fetch CONNECT_ADDR BUNDLE_DIR PACKAGE_ROOT

# Fetch and receive in one step.
vot-cli pull CONNECT_ADDR BUNDLE_DIR DESTINATION_DIR RECEIPT.cbor KEY_SOURCE \
  OBSERVED_AT PACKAGE_ROOT

# Run a rendezvous service (ADR-0033).
vot-cli rendezvous LISTEN_ADDR

# Run a relay, for two ends that cannot punch (ADR-0034).
vot-cli relay LISTEN_ADDR
```
...
vot-cli serve BUNDLE_DIR 0.0.0.0:0            # registers under the bundle's root
vot-cli fetch ROOT BUNDLE_DIR                 # resolves that root, no address
```

Serve certificates are ephemeral by default. Fetch verifies the server identity
only when `VOT_FETCH_SERVE_IDENTITY` pins its certificate digest. Content is
...
vot-cli capability keygen                      # prints the secret, then the public
...
vot-cli capability issue env:ISSUER_SECRET you.example them.example \
...
vot-cli serve BUNDLE_DIR 0.0.0.0:9000
...
vot-cli fetch ADDR BUNDLE_DIR PACKAGE_ROOT
```
...
export VOT_SERVE_ISSUER_NAME=you.example
export VOT_SERVE_AUDIENCE=them.example
vot-cli receive-push 0.0.0.0:9000 RECEIVED_BUNDLES CERT.pem KEY.pem
# Prints: identity HEX (pin with VOT_PUSH_IDENTITY)

vot-cli capability issue-push env:ISSUER_SECRET you.example them.example \
  env:HOLDER_PUBLIC PACKAGE_ROOT PACKAGE_LENGTH 86400 push-token.cbor
export VOT_PUSH_IDENTITY=HEX
vot-cli push BUNDLE_DIR ADDR push-token.cbor env:HOLDER_SECRET
```
...
| `VOT_FETCH_RAILS` | `clamp(2 * cores, 1, 8)`; 1 for direct macOS loopback | Concurrent fetch or push sessions (multi-rail) |
...
| `VOT_FETCH_PROVERS` | unset | Total proving thread ceiling for a fetch, divided evenly across its rails with at least one per rail. The minimum is 1: a fetch proves and places every cover on its provers, so one with none would book no coverage and stall |
| `VOT_STALL_MS` | 30000 | Stall budget in milliseconds: how long either end may go without settling anything before the session ends as stalled. Values under the 11000 floor are ignored, since a starved but healthy host goes that quiet on its own. A fetch settles coverage a whole answered bundle at a time, up to 4 MiB, so it has to place about one bundle per budget or be given up on: roughly 140 KB/s at the default. Raise it for a path slower than that |
...
| `VOT_FETCH_SERVE_IDENTITY` | unset | Pins the serve's identity: the 64 hex characters of the certificate digest `vot-cli serve` prints. The fetch drops any connection whose certificate differs, before it sends anything |
...
| Contents | Meaning |
| --- | --- |
| `ed25519-secret:HEX` | Ed25519 secret, 64 hex chars. Signs receipts or capabilities, or proves holder identity, depending on the command. |

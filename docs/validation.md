...
required: a surviving mutant fails the run rather than being noted. The matrix
also carries the features a package must be built with, because a mutant in a
module the tests never compile is reported missed whatever the tests say. That is
what `vot-object-store` needs `s3-live` for.

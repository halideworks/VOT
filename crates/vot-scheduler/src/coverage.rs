...
impl RangeState {
    pub(super) fn check(&self, covered_offset: u64, bytes: u64) -> Result<Option<Booking>, Error> {
        let active = self;
    ...
    /// Records what [`RangeState::check`] admitted, merging with either
    /// neighbour so the map holds one entry per contiguous covered run.
    pub(super) fn book(&mut self, covered_offset: u64, booking: &Booking) {
        let active = self;
        ...
        if let Some((&earlier_start, &earlier_end)) =
            active.extents.range(..covered_offset).next_back()
        {

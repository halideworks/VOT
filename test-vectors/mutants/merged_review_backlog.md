...
| Windows checkpoint replacement | Use `std::fs::rename` when the compaction destination already exists | `repeated_checkpoints_append_and_replay` runs on native Windows CI; `vot-platform-fs` isolates `MoveFileExW` and retains `unsafe_code = "forbid"` elsewhere |
...
| Resume store million-object capacity | Use a fixed-width whole-store snapshot or rewrite the complete log for every checkpoint | `resume_store_handles_million_small_file_workload` reserves one million objects, checkpoints a large object, and reopens the range-encoded log below the cap |
...
| Owner-blind Careful Resume invalidation | Delete active saved state immediately, or forget an invalidation while its permit remains active | `active_careful_resume_observation_cannot_be_replaced` requires `AlreadyInUse`, defers deletion for every invalidator, and proves the stale observation is gone when the private owner releases it |

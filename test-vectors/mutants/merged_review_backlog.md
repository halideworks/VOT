...
| CLI durability overclaim | Change the publication receipt from Fast/`TRANSIT_VERIFIED` to Balanced/`DURABLE` without a commit-journal durability transition | `publication_receipt_claims_only_performed_assurance` fails; the required `vot-cli` mutation run reports 184 caught, 20 unviable, and zero missed |

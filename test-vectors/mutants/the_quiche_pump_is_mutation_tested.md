...
| `replace size_buffers_reporting_clamps with ()` | optional by design | best-effort buffer tuning and a stderr line, neither of which a test observes |
...
| `replace > with >= in send_all` | equivalent behavior | either release-time comparison, against the pass horizon or against the clock; both agree unless a packet's release falls on that exact instant, and parking a packet due exactly then costs one wake and no bytes |
...
| `replace += with *= in run` | cadence timing | the ramp ping counter frozen at zero keeps the 25 ms cadence for the connection's life instead of a second, a per-pass timing difference no functional test can see; the budget's own edges are pinned by `the_ramp_cadence_falls_silent_at_its_whole_budget` |
| `replace >= with < in run` | cadence timing | the interval comparison inverted spends the whole ping budget in the first milliseconds instead of over a second; the same pings go out either way and only the cadence shape differs |

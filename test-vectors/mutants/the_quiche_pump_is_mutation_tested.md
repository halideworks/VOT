...
| `replace > with >= in send_all` | equivalent behavior | the release-time comparison against the pass horizon; the two agree unless a packet's release falls on that exact instant, and parking a packet due exactly then costs one wake and no bytes |

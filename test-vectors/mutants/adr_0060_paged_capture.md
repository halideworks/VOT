...
With temporary files on ext4, the changed `vot-sdk-file` and `vot-platform-fs` paths were tested with Rust 1.97.1 and cargo-mutants 26.0.0. Of 160 generated mutants, 146 were caught by test failure and 14 did not compile. None survived or timed out. No mutation exclusions were added.
...
CI exposed two timeouts when `scan_offset` was replaced with `Some(0)` or `Some(1)`. An allocated empty page could keep the scan running after other tests detected the wrong return values. Both timeouts were reproduced locally on ext4. The scan body now consumes a checked byte budget, independently of cursor positioning. Both hand-applied mutations fail the capture suite in 0.08 seconds. A cached empty-page regression also checks that reaching EOF preserves the cursor.

## Reproduction

```sh
git diff 84a2402 -- crates/vot-sdk-file crates/vot-platform-fs > changed.diff
TMPDIR=/tmp cargo +1.97.1 mutants --package vot-sdk-file --package vot-platform-fs \
